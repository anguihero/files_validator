...
async def validate_xlsx(file_bytes: bytes, filename: str):
    ...
    try:
        if size_bytes == 0:
            ...
            try:
                ...
                result["message"] = f"El archivo parece estar dañado: {str(e)}"

    except Exception as e:
        result["error_type_09_file"] = True
        result["message"] = f"Error inesperado durante la validación: {str(e)}"
